  ...
  d3.csv("data/participants_info.csv").then(data => {
    ...
    function drawPairedBarChart(data) {
      ...
      const x1 = d3.scaleBand()
        .domain(["leAcuity", "reAcuity"]) 
        .rangeRound([0, x0.bandwidth()])
        .padding(0.05);
      
      const minY = d3.min(data, d => Math.min(d.reAcuity, d.leAcuity) - Math.max(d.reStdErr, d.leStdErr)) * 0.9;
      const maxY = d3.max(data, d => Math.max(d.reAcuity, d.leAcuity) + Math.max(d.reStdErr, d.leStdErr)) * 1.1;
      ...
      svg.append("text")
        ...
        .text("Visual Acuity by Diagnosis: Left Eye vs Right Eye");
    ...
    function drawScatterPlot(data) {
      ...
      svg.append("text")
        ...
        .style("font-size", "16px")
        .style("font-weight", "bold")
        .text("Visual Acuity Scatter Plot: Left Eye vs Right Eye");
    ...
    function drawDifferenceChart(data) {
      ...
      g.selectAll(".bar")
        ...
        .attr("fill", d => d.difference > 0 ? "#e74c3c" : "#4CAF50")  
        .on("mouseover", function(event, d) {
          d3.select(this).attr("opacity", 0.8);
      ...
      svg.append("text")
        ...
        .style("font-size", "16px")
        .style("font-weight", "bold")
        .text("Difference in Visual Acuity by Diagnosis: Left Eye vs Right Eye");
      ...
      svg.append("text")
        ...
        .attr("text-anchor", "middle")
        .style("font-size", "12px")
        .text("Positive values (red) indicate worse vision in the right eye, negative values (green) indicate worse vision in the left eye");
    }
